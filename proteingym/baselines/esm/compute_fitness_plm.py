...
# Minimal, sequence-only fitness computation using Hugging Face protein LMs (e.g., facebook/esm2_*).
# This script intentionally drops all MSA/structure-specific code for simplicity and broader compatibility.


def create_parser():
    parser = argparse.ArgumentParser(description="Compute mutation fitness using Hugging Face protein LMs.")
    parser.add_argument(
        "--batch-size",
        type=int,
        default=16,
    ...
    parser.add_argument(
        ...
        help="Data type for model weights: fp32 (default), bf16, or fp16.",
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        nargs="+",
        required=True,
        help="One or more HF model identifiers or local paths (e.g., facebook/esm2_t33_650M_UR50D)",
    )
...
def _load_hf_model_and_tokenizer(model_name: str, device: torch.device):
...
def _tokenize_seq(tokenizer, seq: str, device: torch.device, add_special_tokens: bool = True):
    return tokenizer(seq, return_tensors="pt", add_special_tokens=add_special_tokens).to(device)


def _log_softmax_logits(model, inputs) -> torch.Tensor:
    # Use autocast for computation (default bf16)
    import builtins
...
def _label_row_hf(row_mut_str: str, sequence: str, token_log_probs: torch.Tensor, tokenizer, offset_idx: int) -> float:
    ...
    for mutation in row_mut_str.split(":"):
        ...
        # +1 for CLS (assumes single CLS and single EOS)
...
def _masked_marginals(
    ...
    for batch_start in _tqdm(range(0, len(positions), batch_size), desc=bar_desc, disable=True):
        ...
        if use_windows:
            ...
            inputs = tokenizer(subseqs, return_tensors="pt", add_special_tokens=True, padding=True)
            inputs = {k: v.to(device) for k, v in inputs.items()}
            for i, lp in enumerate(local_positions):
                inputs["input_ids"][i, 1 + lp] = tokenizer.mask_token_id
            logp = _log_softmax_logits(model, inputs)
        else:
            seqs = [sequence for _ in batch_positions]
            inputs = tokenizer(seqs, return_tensors="pt", add_special_tokens=True, padding=True)
            inputs = {k: v.to(device) for k, v in inputs.items()}
            for i, pos in enumerate(batch_positions):
                inputs["input_ids"][i, 1 + pos] = tokenizer.mask_token_id
            logp = _log_softmax_logits(model, inputs)
    ...
    # Shape [1, L+2, V] to match downstream indexing with +1 offset
...
def main(args):
    # Set batch size and autocast dtype globally
    import builtins
    ...
    for model_name in args.model_name_or_path:
        tokenizer, model = _load_hf_model_and_tokenizer(model_name, device)
    ...
    for dms_idx in tqdm(dms_indices, desc="DMS"):
        ...
        for model_name, tokenizer, model in tokenizers_models:
            ...
            if args.scoring_strategy == "wt-marginals":
                ...
                if inputs["input_ids"].shape[1] > max_len and args.scoring_window == "optimal":
                    ...
                    for _ in _tqdm([0], desc=bar_desc):
        ...
        if len(args.model_name_or_path) > 1:
            ...
            if present:
                df["Ensemble_PLM"] = df[present].mean(axis=1)
                df.to_csv(dms_output_file, index=False)
