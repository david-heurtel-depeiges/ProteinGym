...
# Aggregate AMPLIFY model results into ProteinGym performance CSVs
...
# This should be the parent folder containing subdirectories for each model
# The script will recursively find all models using the config file
INPUT_SCORES_BASE="${SCRATCH_BASE}/results/"

# --- Model configuration ---
# Generate this using: python generate_amplify_config.py --score-folder <folder> --output <config>
CUSTOM_CONFIG="${REPO_ROOT}/config_thematic_lab.json"

# --- Output directories ---
MERGED_SCORES_DIR="${SCRATCH_BASE}/results/merged_scores/substitutions"
OUTPUT_PERFORMANCE_FOLDER="${SCRATCH_BASE}/results/performance/all_models"
...
if [[ ! -f "${CUSTOM_CONFIG}" ]]; then
    ...
    echo "    --score-folder ${INPUT_SCORES_BASE} \\"
...
echo "================================================================================"
echo "STEP 1: MERGING MODEL SCORES"
echo "================================================================================"
...
python "${REPO_ROOT}/proteingym/merge_recursive.py" \
